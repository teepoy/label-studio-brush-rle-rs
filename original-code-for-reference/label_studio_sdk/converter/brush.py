...
DecodeFunc = Callable[[Any, bool], np.ndarray]
EncodeFunc = Callable[
    [Sequence[int], Optional[int], Optional[Sequence[int]]], list[int]
]

try:
    from label_studio_brush_rle_rs import (  # type: ignore[import-untyped]
        decode_rle as _decode_rle_rust_raw,
        encode_rle as _encode_rle_rust_raw,
    )
except Exception:  # pragma: no cover - rust module optional in tests
    _decode_rle_rust: Optional[DecodeFunc] = None
    _encode_rle_rust: Optional[EncodeFunc] = None
else:
    _decode_rle_rust = cast(DecodeFunc, _decode_rle_rust_raw)
    _encode_rle_rust = cast(EncodeFunc, _encode_rle_rust_raw)

_DECODE_RLE_RUST_AVAILABLE = _decode_rle_rust is not None
_ENCODE_RLE_RUST_AVAILABLE = _encode_rle_rust is not None
...
def _decode_rle_python(rle, print_params: bool = False):
...
def decode_rle(rle, print_params: bool = False):
    """Decode RLE using the Rust extension when available, falling back to Python."""

    if _decode_rle_rust is not None:
        return _decode_rle_rust(rle, print_params)

    return _decode_rle_python(rle, print_params)
...
def _encode_rle_python(arr, wordsize=8, rle_sizes=None):
...
def encode_rle(arr, wordsize=8, rle_sizes=None):
    """Encode array to RLE using Rust extension when available."""

    if _ENCODE_RLE_RUST_AVAILABLE:
        rle_sizes = [3, 4, 8, 16] if rle_sizes is None else list(rle_sizes)
        rust_result = _encode_rle_rust(arr, wordsize, rle_sizes)
        return list(rust_result)

    return _encode_rle_python(arr, wordsize, rle_sizes)
