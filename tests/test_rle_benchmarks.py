...
def sample_mask_flattened() -> np.ndarray:
    rng = np.random.default_rng(42)
    mask = (rng.random((256, 256)) > 0.7).astype(np.uint8) * 255
    return np.repeat(mask.ravel(), 4)


@pytest.fixture(scope="module")
def sample_rle(sample_mask_flattened: np.ndarray) -> list[int]:
    return _brush.encode_rle(sample_mask_flattened)
...
@pytest.mark.benchmark
def test_encode_rle_benchmark(sample_mask_flattened):
    if getattr(_brush, "_encode_rle_rust", None) is None:
        pytest.skip("Rust encode_rle wrapper not available")

    python_rle = _brush._encode_rle_python(sample_mask_flattened)
    rust_rle = _brush.encode_rle(sample_mask_flattened)
    assert list(rust_rle) == list(python_rle)

    _brush._encode_rle_python(sample_mask_flattened)
    _brush.encode_rle(sample_mask_flattened)

    python_time = _measure(
        lambda: _brush._encode_rle_python(sample_mask_flattened), repeats=10
    )
    rust_time = _measure(lambda: _brush.encode_rle(sample_mask_flattened), repeats=10)

    speedup = python_time / rust_time if rust_time else float("inf")
    print(
        f"Rust encode_rle is {speedup:.2f}x faster than Python baseline "
        f"(Python {python_time * 1000:.3f} ms vs Rust {rust_time * 1000:.3f} ms)"
    )

    assert rust_time <= python_time * 1.05
